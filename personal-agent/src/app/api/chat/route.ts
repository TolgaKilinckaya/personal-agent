  ...
  try {
    ...
    if (!openaiApiKey) {
      return NextResponse.json(
        {
          answer:
            "The AI backend is not configured yet (missing OPENAI_API_KEY). Please add it to your environment."
        },
        { status: 200 }
      );
    }
...
`;
    ...
    return NextResponse.json({ answer, sources: scored.map((s) => s.chunk.id) });
  } catch (err) {
    console.error(err);
    return NextResponse.json(
